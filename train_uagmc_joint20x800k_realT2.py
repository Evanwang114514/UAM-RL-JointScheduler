...
Joint action is deliberately tiny. The actual T2 environment exposes two passenger\nactions and aircraft has {Hold,V0,V1}; joint rows use a single Discrete(6) pair action.  The PPO policy
...
Controls
--------
- T2 only: V0,V1 -> V2. Passenger control preserves the environment's actual\n  Discrete(2) labels; aircraft control is {Hold,V0,V1}.
...
class JointActionWrapper(gym.Wrapper):
    """
    Decode the real T2 passenger action space x aircraft {Hold,V0,V1}.

    IMPORTANT:
    core.make_env(... candidate_from_vertiports=[0,1], to_vertiport=2)
    exposes Discrete(2) here.  Therefore the passenger branch must preserve the
    underlying two labels exactly; no synthetic Ground action is inserted.
    Joint action = 2 passenger labels x 3 aircraft labels = Discrete(6).
    """
    def __init__(self, env: gym.Env, *, row: str):
        ...
        if self.passenger_n != len(CANDIDATES):
            raise RuntimeError(
                f"T2 passenger action mismatch: underlying={env.action_space}, "
                f"expected Discrete({len(CANDIDATES)}) for candidates={list(CANDIDATES)}"
            )
        self.aircraft_n = 3
    ...
    def step(self, action):
        ...
        pair_idx = int(passenger_action * self.aircraft_n + aircraft_action)
        if len(out) == 5:
            obs, reward, terminated, truncated, info = out
            info = dict(info)
            info.update({
                "joint_pair_action": pair_idx,
...
class JointPairHead(nn.Module):
    """
    Output 6 pair logits = 2 real passenger actions x 3 aircraft actions.

    J0: separate passenger/aircraft scorers, parallel factorization.
    J1: unified role-conditioned scorer, parallel factorization.
    J2: separate passenger scorer + passenger-conditioned aircraft scorer.
    J3: unified role-conditioned scorer + passenger-conditioned aircraft scorer.
    """
    P_N = 2
    A_N = 3
    PAIR_N = 6
    ...
    def __init__(self, latent_dim: int, mode: str):
        ...
        if self.mode == "J0":
            self.passenger = nn.Linear(latent_dim, self.P_N)
            self.aircraft = nn.Linear(latent_dim, self.A_N)
        elif self.mode == "J2":
            self.passenger = nn.Linear(latent_dim, self.P_N)
            self.aircraft_cond = nn.Linear(latent_dim, self.P_N * self.A_N)
        elif self.mode in ("J1", "J3"):
            # Unified scorer uses a common three-slot action code:
            # passenger labels 0/1 occupy slots 0/1; aircraft labels 0/1/2 use all 3.
            # commit code is two-dimensional because only passenger choice is committed.
            self.shared = nn.Sequential(
                nn.Linear(latent_dim + 1 + 3 + self.P_N, 128),
                nn.ReLU(),
                nn.Linear(128, 1),
            )
        else:
    ...
    def _shared_scores(
        ...
        *,
        ...
        for a in range(int(n_actions)):
            ...
            action_oh[:, a] = 1.0
            commit_oh = torch.zeros(B, self.P_N, dtype=latent.dtype, device=latent.device)
    ...
    def forward(self, latent: torch.Tensor) -> torch.Tensor:
        if self.mode == "J0":
            p = self.passenger(latent)                    # [B,2]
            a = self.aircraft(latent)                     # [B,3]
            return (p.unsqueeze(2) + a.unsqueeze(1)).reshape(-1, self.PAIR_N)

        if self.mode == "J2":
            p = self.passenger(latent)                    # [B,2]
            a = self.aircraft_cond(latent).reshape(-1, self.P_N, self.A_N)
            return (p.unsqueeze(2) + a).reshape(-1, self.PAIR_N)

        p = self._shared_scores(
            latent, role_sign=+1.0, n_actions=self.P_N, commit=None
        )                                                 # [B,2]
        if self.mode == "J1":
            a = self._shared_scores(
                latent, role_sign=-1.0, n_actions=self.A_N, commit=None
            )                                             # [B,3]
            return (p.unsqueeze(2) + a.unsqueeze(1)).reshape(-1, self.PAIR_N)

        pair_rows = []
        for p_idx in range(self.P_N):
            a_given_p = self._shared_scores(
                latent, role_sign=-1.0, n_actions=self.A_N, commit=p_idx
            )
            pair_rows.append(p[:, p_idx:p_idx + 1] + a_given_p)
        return torch.stack(pair_rows, dim=1).reshape(-1, self.PAIR_N)


class JointPairPolicy(ActorCriticPolicy):
    """SB3 policy with the real T2 Discrete(6) factorized joint-action head."""
    def __init__(
        self,
        observation_space,
        action_space,
        ...
        **kwargs,
        ...
        if not isinstance(action_space, spaces.Discrete) or int(action_space.n) != 6:
            raise RuntimeError(f"JointPairPolicy requires Discrete(6), got {action_space}")
...
def evaluate_checkpoint_with_joint_shares(**kwargs) -> Dict[str, Any]:
    ...
    pair_shares = [fnum(result.get(f"action_{i}_share"), 0.0) for i in range(6)]
    for p in range(2):
    ...
    for a in range(3):
        result[f"aircraft_action_{a}_share"] = float(
            sum(pair_shares[p * 3 + a] for p in range(2))
        )
    return result
...
def run_matrix(args: argparse.Namespace) -> Path:
    ...
    manifest = {
        "experiment": "UAGMC_JOINT20_REAL_T2" if requested_steps == 800_000 else "UAGMC_JOINT20_REAL_T2_SUBSET",
        ...
        "joint": {
            ...
            "passenger_actions": ["underlying_T2_action_0", "underlying_T2_action_1"],
            "joint_action_space": "Discrete(6) = real T2 Discrete(2) passenger x {Hold,V0,V1}; no synthetic Ground label",
