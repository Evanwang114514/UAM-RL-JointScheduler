...
Profiles (same global rollout=20,480 and batch=2,048 for a fair speed test):
    P0:  5 envs x 4096, batch 2048
    P1:  8 envs x 2560, batch 2048
    P2: 10 envs x 2048, batch 2048
    P3: 16 envs x 1280, batch 2048

All n_env values divide 50,000 exactly so formal 50k checkpoints remain exact.
The maximum is capped at 16 Windows spawn workers to avoid pagefile/DLL failures.
...
import os
# Windows SubprocVecEnv uses spawn; keep each worker single-threaded so 16
# workers do not multiply BLAS/OpenMP thread pools or pagefile pressure.
...
SPEED_PROFILES = (
    # Fair throughput comparison: identical global rollout (20,480),
    # identical batch size (2,048), identical PPO epochs; only CPU env
    # parallelism changes. 16 workers is the hard Windows safety ceiling.
    SpeedProfile("P0_5x4096_b2048", 5, 4096, 2048),
    SpeedProfile("P1_8x2560_b2048", 8, 2560, 2048),
    SpeedProfile("P2_10x2048_b2048", 10, 2048, 2048),
    SpeedProfile("P3_16x1280_b2048", 16, 1280, 2048),
)
...
def _ensure_t3_charge_rate() -> float:
    """Add the injected V3 to the charge-rate tables used by eVTOLBuilder.

    Public UAGMC defines VERTIPORT_CHARGE_RATE only for keys 0/1/2, while
    eVTOLBuilder indexes that table directly during fixed-fleet spawning.
    T3 is an added departure vertiport, so we deliberately give it V1's
    charge rate (same small-origin class) rather than inventing a new value.
    The patch is process-local and therefore safe under Windows spawn.
    """
    try:
        import at_obj.vertiport.vertiport_spec as vp_spec
        import at_obj.evtol.evtol_builder as evtol_builder_mod
    except Exception as exc:
        raise RuntimeError(
            "Cannot import UAGMC vertiport/eVTOL spec modules for T3 patch"
        ) from exc

    table = getattr(vp_spec, "VERTIPORT_CHARGE_RATE", None)
    if not isinstance(table, dict):
        raise RuntimeError("VERTIPORT_CHARGE_RATE is missing or not a dict")

    if "3" not in table:
        if "1" in table:
            table["3"] = float(table["1"])
        elif "2" in table:
            table["3"] = float(table["2"])
        elif "0" in table:
            table["3"] = float(table["0"])
        else:
            raise RuntimeError(
                "Cannot derive V3 charge rate: no reference key 0/1/2 exists"
            )

    # evtol_builder imports the dict into module scope. Usually this is the
    # same object, but set it explicitly in case a local copy is present.
    builder_table = getattr(evtol_builder_mod, "VERTIPORT_CHARGE_RATE", None)
    if isinstance(builder_table, dict):
        builder_table["3"] = float(table["3"])
...
def install_topology_patch(topology: str) -> None:
    ...
    def _vp_init_t3(self, *args, **kwargs):
        ...
        # V3 is a small departure origin like V1, so use the same nominal
        # constructor capacity metadata. The conserved fleet is allocated
        # explicitly after reset; this value is not used to create extra fleet.
        ref_capacity = int(self.vertiport_evtol_capacity.get("1", 1))
        self.vertiport_evtol_capacity["3"] = max(1, ref_capacity)
...
def normalize_t3_initial_allocation(
    ...
    """Normalize the conserved fleet exactly to 50%/25%/25% on V0/V1/V3.

    The old implementation only filled V3. With the actual N=40 fixed-fleet
    reset (e.g. 32/8/0), that produced 22/8/10 and then failed the assertion.
    This implementation performs a generic surplus -> deficit rebalance over
    every local vertiport, so 32/8/0 becomes exactly 20/10/10 while preserving
    the total fleet and updating each moved aircraft's fixed home.
    """
    if "3" not in scenario.vertiports.vertiport_list:
        raise RuntimeError("T3 map injection failed: V3 is absent")

    evtols_by_vp = scenario.vertiports.evtols_at_vertiport
    for vid in ("0", "1", "2", "3"):
        evtols_by_vp.setdefault(vid, [])
    ...
    # At reset all N aircraft must be local. Destination V2 (or any future
    # extra key) has target zero; if it contains aircraft, it becomes a donor.
    all_vids = sorted(
        set(str(v) for v in evtols_by_vp.keys())
        | set(desired_origins.keys())
    )
    target = {
        vid: int(desired_origins.get(vid, 0))
        for vid in all_vids
    }

    total_local_before = sum(len(evtols_by_vp.get(vid, [])) for vid in all_vids)
    if total_local_before != int(fleet_size):
        raise RuntimeError(
            f"T3 reset must have all conserved aircraft local before rebalance: "
            f"local={total_local_before}, N={fleet_size}"
        )

    def counts() -> Dict[str, int]:
        return {
            vid: len(evtols_by_vp.get(vid, []))
            for vid in all_vids
        }

    before = counts()
    deficits = {
        vid: target[vid] - before[vid]
        for vid in all_vids
        if target[vid] - before[vid] > 0
    }
    surpluses = {
        vid: before[vid] - target[vid]
        for vid in all_vids
        if before[vid] - target[vid] > 0
    }

    if sum(deficits.values()) != sum(surpluses.values()):
        raise RuntimeError(
            f"T3 rebalance is not mass-conserving: before={before}, "
            f"target={target}, deficits={deficits}, surpluses={surpluses}"
        )

    donor_pool: List[Tuple[str, Any]] = []
    for source in all_vids:
        surplus = int(surpluses.get(source, 0))
        if surplus <= 0:
            continue
        local = [
            e
            for e in evtols_by_vp.get(source, [])
            if state_name(e) != "FLYING"
        ]
        local.sort(key=lambda e: str(getattr(e, "id", "")))
        if len(local) < surplus:
            raise RuntimeError(
                f"T3 donor V{source} has only {len(local)} movable aircraft "
                f"for required surplus {surplus}"
            )
        donor_pool.extend((source, e) for e in local[:surplus])

    cursor = 0
    for dest in ("1", "3", "0", "2"):
        need = int(deficits.get(dest, 0))
        for _ in range(need):
            if cursor >= len(donor_pool):
                raise RuntimeError(
                    f"T3 donor pool exhausted: before={before}, target={target}"
                )
            source, evtol = donor_pool[cursor]
            cursor += 1
    ...
    # Handle any future/unknown deficit keys deterministically too.
    for dest in all_vids:
        if dest in {"0", "1", "2", "3"}:
            continue
        need = int(deficits.get(dest, 0))
        for _ in range(need):
            if cursor >= len(donor_pool):
                raise RuntimeError("T3 donor pool exhausted on extra key")
            source, evtol = donor_pool[cursor]
            cursor += 1
            _move_evtol_between_vertiports(scenario, evtol, source, dest)

    final_all = counts()
    final_origins = {
        vid: final_all.get(vid, 0)
        for vid in ("0", "1", "3")
    }

    if final_origins != desired_origins:
        raise RuntimeError(
            f"T3 initial allocation mismatch after generic rebalance: "
            f"before={before}, final={final_origins}, desired={desired_origins}"
        )

    if sum(final_all.values()) != int(fleet_size):
        raise RuntimeError(
            f"T3 fleet conservation failed: final={final_all}, N={fleet_size}"
        )

    # No aircraft should remain at non-origin locations at formal reset.
    off_origin = {
        vid: n
        for vid, n in final_all.items()
        if vid not in desired_origins and n != 0
    }
    if off_origin:
        raise RuntimeError(
            f"T3 reset left aircraft outside V0/V1/V3: {off_origin}"
        )

    if hasattr(scenario, "_fixed_initial_allocation"):
        try:
            scenario._fixed_initial_allocation = dict(final_origins)
        except Exception:
            pass

    scenario._t3_allocation_before_rebalance = dict(before)
    scenario._t3_allocation_after_rebalance = dict(final_origins)
...
# =============================================================================
# Formal runtime preflight (single process, before any Windows spawn benchmark)
# =============================================================================
...
def run_formal_preflight(
    *,
    ...
    """Exercise the heaviest E6/O10/T3 env once without subprocess workers.

    This catches T3 map/config/allocation errors before spawning 5-16 Python
    workers. It is a structural check only and does not consume formal PPO
    training budget.
    """
    run_dir = root / "_preflight"
    ...
    try:
        ...
        if allocation != expected:
            raise RuntimeError(
                f"Preflight T3 allocation mismatch: got={allocation}, expected={expected}"
            )

        charge_rate_v3 = _ensure_t3_charge_rate()

        # One legal step verifies that reset-time topology/observation plumbing
        # survives an actual transition as well.
        ...
        result = {
            ...
            "charge_rate_v3": float(charge_rate_v3),
            "observation_shape": list(np.asarray(obs).shape),
            "elapsed_seconds": time.time() - started,
        }
        write_json(run_dir / "preflight.json", result)
        print(
            f"PREFLIGHT PASS | T3 allocation={allocation} | "
            f"V3 charge={charge_rate_v3:.3f} | obs={result['observation_shape']}"
        )
        return result
...
def select_fastest_profile(
    *,
    ...
    rollout_sizes = {p.n_envs * p.n_steps for p in SPEED_PROFILES}
    batch_sizes = {p.batch_size for p in SPEED_PROFILES}
    if rollout_sizes != {20_480}:
        raise RuntimeError(f"Speed profiles must share rollout=20480, got {rollout_sizes}")
    if batch_sizes != {2_048}:
        raise RuntimeError(f"Speed profiles must share batch=2048, got {batch_sizes}")
    if max(p.n_envs for p in SPEED_PROFILES) > 16:
        raise RuntimeError("Windows formal benchmark forbids >16 spawn workers")
    ...
    for profile in SPEED_PROFILES:
        ...
        # Give Windows spawn workers/DLL mappings time to release before the
        # next profile. This reduces transient pagefile pressure.
        gc.collect()
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        time.sleep(1.0)
...
def main() -> int:
    ...
    manifest = {
        "experiment": "UAGMC_E3_E6_OBSERVATION_TOPOLOGY_MATRIX_FORMAL",
        ...
        "T3_charge_rate_rule": "clone V1 charge rate for injected V3",
        ...
        "speed_selection": (
            "single-process E6/O10/T3 preflight, then benchmark 5/8/10/16 env "
            "profiles with identical rollout=20480 and batch=2048; choose highest actual FPS"
        ),
        "speed_profiles": [asdict(p) for p in SPEED_PROFILES],
        "ppo_controls": {
            "n_epochs": base.N_EPOCHS,
            "gamma": base.GAMMA,
            "gae_lambda": base.GAE_LAMBDA,
    ...
    # Fail fast in ONE process before spawning benchmark workers.
    ...
    if (
        ...
        # Explicit skip means use the previously proven-safe maximum worker
        # profile, not the slowest benchmark candidate.
        profile = next(p for p in SPEED_PROFILES if p.n_envs == 16)
        write_json(
            selected_path,
            {
        ...
        print(
            f"Benchmark skipped; use safe 16-worker profile {profile.name}"
        )
    else:
