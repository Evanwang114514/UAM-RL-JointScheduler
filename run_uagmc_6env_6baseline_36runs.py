# -*- coding: utf-8 -*-
"""
EXACT 6 environments x 6 analytical/rule baselines = 36 runs.
...
def parse_args():
    p = argparse.ArgumentParser(
        description=(
            "EXACT 6 environments x 6 online-legal analytical baselines = "
            "36 CPU episodes. One fixed evaluation seed only."
        )
    )
    ...
    p.add_argument(
        ...
        help="single fixed evaluation seed; default 123",
    )
...
def main() -> int:
    ...
    root = (
        ...
        if args.output_root
            ...
            ROOT
            / "serial_runs"
            / f"uagmc_6env_6baseline_36runs_{args.topology}_seed{seed}_{stamp}"
        ).resolve()
    )
    root.mkdir(parents=True, exist_ok=True)

    manifest = {
        "experiment": "UAGMC_6ENV_6BASELINE_EXACT_36_RUNS",
        "created": datetime.now().isoformat(timespec="seconds"),
        "stages": stages,
        "methods": methods,
        "method_aliases": METHOD_ALIASES,
        "topology": args.topology,
        "single_eval_seed": seed,
        "total_runs": total,
        "design": "one episode per environment-method cell; no seed replication",
        ...
        "causality": (
            "no unrevealed future passenger requests; "
            "no uncommitted future reposition actions"
        ),
        "method_descriptions": {
            ...
            "CSM": (
                "discrete FIFO committed-passenger / committed-supply "
                "matching to predicted departure"
            ),
            "ECTF": (
                "CSM + service-flight time + current origin/destination "
                "TLOF feasibility"
            ),
            "MPTC": (
                "ECTF focal completion + marginal future waiting imposed "
                "on already known passengers"
            ),
        },
    ...
    print("=" * 126)
    print("UAGMC 6 ENVIRONMENTS x 6 BASELINES = EXACTLY 36 RUNS")
    print(f"Topology={args.topology} | seed={seed} | total={total} | CPU-only")
    ...
    for stage in stages:
        for method in methods:
            idx += 1
            run_dir = root / f"{stage}__{method}"
            run_dir.mkdir(parents=True, exist_ok=True)

            print(
                f"[{idx:02d}/{total:02d}] {stage} / {method} / seed={seed}",
                flush=True,
            )
            ...
            print(
                ...
                f"finish={int(row.get('N_finished', 0))}/"
                f"{int(row.get('N', 0))} | "
    ...
    if idx != total:
        raise RuntimeError(f"run-count mismatch: executed={idx}, expected={total}")
    ...
    summary_lines = [
        "UAGMC 6 ENV x 6 BASELINE = EXACT 36-RUN SUMMARY",
        "=" * 112,
        f"topology={args.topology} | seed={seed} | total_runs={total}",
        "stage | method | ATT | AWT | completion | Jsys/N",
    ]

    for stage in stages:
        stage_rows = [r for r in agg if r["stage"] == stage]
        by_method = {str(r["method"]): r for r in stage_rows}
        for method in methods:
            ...
            summary_lines.append(
                ...
                f"{fnum(r.get('ATT_mean')):8.3f} | "
                f"{fnum(r.get('AWT_mean')):8.3f} | "
    ...
    print("\nDONE")
    print(f"Executed EXACTLY {idx} runs.")
