...
python train_uam_60m_literature_matrix_v3.py --device cuda --profile P3

Resume
------
python train_uam_60m_literature_matrix_v3.py --resume-root "serial_runs\<run>" --device cuda --profile P3

Deferred joint evaluation only
------------------------------
python train_uam_60m_literature_matrix_v3.py --resume-root "serial_runs\<run>" --joint-eval-only
"""
...
class LiteratureObservationWrapper(gym.Wrapper):
    def __init__(self, env: gym.Env, env_key: str, method_id: str):
        ...
        self.spec = ENV_SPECS[self.env_key]
    ...
    def _transform(self, obs: np.ndarray) -> np.ndarray:
        ...
        resource = _resource_vector(scenario, self.spec)
        focal = _focal_od(person)
        tau, shared, srac, gamma, cqm = _shared_srac_gamma_cqm(
            scenario, self.spec, person
        )
        event = _pack_events(scenario, tau)
        tdm, first = _tdm_and_first(scenario, self.spec, tau)
        pair_virtual = _pair_virtual_features(scenario, self.spec, tau)
...
class LiteratureExtractor(BaseFeaturesExtractor):
    def __init__(
        ...
        self.env_key = str(env_key).upper()
        self.spec = ENV_SPECS[self.env_key]
        self.n_actions = self.spec.n_actions
    ...
    def _selected_tau(self, obs: torch.Tensor, action: int) -> torch.Tensor:
        ...
        p = int(action) if not self.spec.joint else int(action) // 2
    ...
    def _future_pairs(self, ext: LiteratureExtractor):
        ...
        for t in range(n_steps - 1):
            for e in range(n_envs):
                a = int(acts[t, e])
                p = a if not ext.spec.joint else a // 2
                p = min(max(p, 0), N_CANDIDATES - 1)
