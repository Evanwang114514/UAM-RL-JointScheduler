...
Joint action is deliberately tiny. The passenger branch uses the actual T2 action
space exposed by the validated UAGMC wrapper (currently Discrete(2): {V0,V1}); the
aircraft branch has {Hold,V0,V1}. The joint wrapper therefore exposes Discrete(6)
in the current environment, while remaining compatible with a future Discrete(3)
passenger wrapper. The PPO policy factorizes the pair logits according to J0/J1/
J2/J3, so the environment never receives a per-aircraft action vector. At most one
eligible empty hub aircraft is repositioned at a passenger decision epoch.
...
Controls
--------
- T2 only: V0,V1 -> V2. The validated passenger environment currently exposes
  {V0,V1}; aircraft control exposes {Hold,V0,V1}. Unified rows share the scorer
  on the common V0/V1 candidate semantics, while Hold remains aircraft-only.
...
# =============================================================================
# Joint scheduling: small factorized passenger x aircraft action + four policy factorizations
# =============================================================================
...
class JointActionWrapper(gym.Wrapper):
    """
    Factorized passenger x aircraft action wrapper.

    Current validated T2 passenger environment exposes Discrete(2) = {V0,V1}.
    Aircraft control is always Discrete(3) = {Hold,V0,V1}. Therefore the current
    joint action space is Discrete(6). If a future passenger wrapper explicitly
    adds Ground and exposes Discrete(3), this wrapper automatically becomes
    Discrete(9) without changing the aircraft side.
    """
    AIRCRAFT_N = 3
    ...
    def __init__(self, env: gym.Env, *, row: str):
        ...
        self.passenger_n = int(env.action_space.n)
        if self.passenger_n not in (2, 3):
            raise RuntimeError(
                "FORMAL45 joint design supports the validated T2 passenger space "
                "Discrete(2)={V0,V1}, or an explicit Ground-enabled "
                "Discrete(3)={Ground,V0,V1}; got "
                f"{env.action_space}."
            )
        self.aircraft_n = self.AIRCRAFT_N
        self.action_space = spaces.Discrete(self.passenger_n * self.aircraft_n)
        self.observation_space = env.observation_space

    def decode(self, action: Any) -> Tuple[int, int]:
        idx = int(np.asarray(action).reshape(-1)[0])
        n_pair = self.passenger_n * self.aircraft_n
        if idx < 0 or idx >= n_pair:
            raise ValueError(f"joint action {idx} outside [0,{n_pair})")
        return idx // self.aircraft_n, idx % self.aircraft_n
    ...
    def step(self, action):
        ...
        pair_action = int(passenger_action * self.aircraft_n + aircraft_action)
...
class JointPairHead(nn.Module):
    """
    Factorized joint-action logits for J0/J1/J2/J3.

    passenger_n is inferred from the environment:
      - current validated T2: 2 = {V0,V1}
      - optional Ground-enabled variant: 3 = {Ground,V0,V1}
    Aircraft action is always 3 = {Hold,V0,V1}.

    For unified J1/J3 with passenger_n=2, passenger labels are mapped to semantic
    slots {V0,V1} -> {1,2}; aircraft uses {Hold,V0,V1} -> {0,1,2}. Thus V0/V1
    truly share the same role-conditioned scorer while Hold remains aircraft-only.
    """
    AIRCRAFT_N = 3
    SEMANTIC_N = 3  # {null/hold, V0, V1}

    def __init__(self, latent_dim: int, mode: str, passenger_n: int):
        super().__init__()
        self.mode = str(mode).upper()
        self.passenger_n = int(passenger_n)
        self.aircraft_n = self.AIRCRAFT_N
        if self.passenger_n not in (2, 3):
            raise ValueError(f"unsupported passenger_n={self.passenger_n}")

        if self.mode == "J0":
            self.passenger = nn.Linear(latent_dim, self.passenger_n)
            self.aircraft = nn.Linear(latent_dim, self.aircraft_n)
        elif self.mode == "J2":
            self.passenger = nn.Linear(latent_dim, self.passenger_n)
            self.aircraft_cond = nn.Linear(
                latent_dim, self.passenger_n * self.aircraft_n
            )
        elif self.mode in ("J1", "J3"):
            # One scorer shared by demand(+1) and supply(-1) roles.
            # Inputs: latent + role_sign + semantic_action_onehot(3)
            #         + passenger_commit_onehot(3).
            self.shared = nn.Sequential(
                nn.Linear(latent_dim + 1 + self.SEMANTIC_N + self.SEMANTIC_N, 128),
                nn.ReLU(),
                nn.Linear(128, 1),
            )
        else:
    ...
    def _passenger_semantic_ids(self) -> List[int]:
        # Current T2 passenger actions {V0,V1} align with semantic slots 1,2.
        # A future Ground-enabled Discrete(3) naturally uses 0,1,2.
        return [1, 2] if self.passenger_n == 2 else [0, 1, 2]
    ...
    def _shared_scores(
        ...
        *,
        ...
        commit_semantic: Optional[int],
    ) -> torch.Tensor:
        B = latent.shape[0]
        rows = []
        for semantic_id in action_ids:
            role = torch.full(
                (B, 1), float(role_sign), dtype=latent.dtype, device=latent.device
            )
            action_oh = torch.zeros(
                B, self.SEMANTIC_N, dtype=latent.dtype, device=latent.device
            )
            action_oh[:, int(semantic_id)] = 1.0
            commit_oh = torch.zeros(
                B, self.SEMANTIC_N, dtype=latent.dtype, device=latent.device
            )
            if commit_semantic is not None:
                commit_oh[:, int(commit_semantic)] = 1.0
            x = torch.cat([latent, role, action_oh, commit_oh], dim=1)
            rows.append(self.shared(x))
        return torch.cat(rows, dim=1)

    def forward(self, latent: torch.Tensor) -> torch.Tensor:
        P, A = self.passenger_n, self.aircraft_n
        pair_n = P * A

        if self.mode == "J0":
            p = self.passenger(latent)                    # [B,P]
            a = self.aircraft(latent)                     # [B,3]
            return (p.unsqueeze(2) + a.unsqueeze(1)).reshape(-1, pair_n)

        if self.mode == "J2":
            p = self.passenger(latent)                    # [B,P]
            a = self.aircraft_cond(latent).reshape(-1, P, A)
            return (p.unsqueeze(2) + a).reshape(-1, pair_n)

        passenger_sem = self._passenger_semantic_ids()
        aircraft_sem = [0, 1, 2]
        p = self._shared_scores(
            latent,
            role_sign=+1.0,
            action_ids=passenger_sem,
            commit_semantic=None,
        )

        if self.mode == "J1":
            a = self._shared_scores(
                latent,
                role_sign=-1.0,
                action_ids=aircraft_sem,
                commit_semantic=None,
            )
            return (p.unsqueeze(2) + a.unsqueeze(1)).reshape(-1, pair_n)

        # J3: aircraft scorer conditions on the sampled/paired passenger commitment.
        pair_rows = []
        for p_local, p_sem in enumerate(passenger_sem):
            a_given_p = self._shared_scores(
                latent,
                role_sign=-1.0,
                action_ids=aircraft_sem,
                commit_semantic=p_sem,
            )
            pair_rows.append(p[:, p_local:p_local + 1] + a_given_p)
        return torch.stack(pair_rows, dim=1).reshape(-1, pair_n)


class JointPairPolicy(ActorCriticPolicy):
    """SB3 policy with a small factorized passenger x aircraft action head."""
    def __init__(
        self,
        observation_space,
        action_space,
        ...
        **kwargs,
        ...
        if not isinstance(action_space, spaces.Discrete):
            raise RuntimeError(f"JointPairPolicy requires Discrete action, got {action_space}")
        pair_n = int(action_space.n)
        if pair_n % 3 != 0 or pair_n not in (6, 9):
            raise RuntimeError(
                f"JointPairPolicy expects Discrete(6) or Discrete(9), got {action_space}"
            )
        passenger_n = pair_n // 3
        self.action_net = JointPairHead(
            int(self.mlp_extractor.latent_dim_pi),
            self.joint_mode,
            passenger_n=passenger_n,
        )
...
def evaluate_checkpoint_with_joint_shares(**kwargs) -> Dict[str, Any]:
    ...
    # Infer the pair action count from the evaluator output. Current T2 joint
    # rows use 2 passenger actions x 3 aircraft actions = 6. Ground-enabled
    # variants may use 3 x 3 = 9.
    pair_n = 0
    while f"action_{pair_n}_share" in result:
        pair_n += 1
    if pair_n not in (6, 9):
        # Keep analysis non-fatal even if the upstream evaluator omits shares.
        pair_n = 6
    passenger_n = pair_n // 3
    pair_shares = [fnum(result.get(f"action_{i}_share"), 0.0) for i in range(pair_n)]
    for p in range(passenger_n):
        result[f"passenger_action_{p}_share"] = float(
            sum(pair_shares[p * 3 + a] for a in range(3))
        )
    for a in range(3):
        result[f"aircraft_action_{a}_share"] = float(
            sum(pair_shares[p * 3 + a] for p in range(passenger_n))
        )
    result["joint_passenger_n"] = int(passenger_n)
    result["joint_pair_n"] = int(pair_n)
    return result
...
def run_matrix(args: argparse.Namespace) -> Path:
    ...
    if args.resume_root:
        ...
        root = (
            ROOT
            / "serial_runs"
            / f"uagmc_45x{requested_steps//1000}k_T2_seed{args.seed}_{stamp}"
        ).resolve()
    ...
    manifest = {
        "experiment": "UAGMC_FORMAL_45x800K" if requested_steps == 800_000 else "UAGMC_FORMAL45_SUBSET",
        ...
        "joint": {
            ...
            "joint_action_space": "passenger Discrete(2 current T2) x aircraft Discrete(3); pair Discrete(6), no per-aircraft vector",
